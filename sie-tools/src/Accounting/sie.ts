...
export const writeSie = async (fileNameBase: string, vers: Verification[], A: number, B: number, C: number, D: number, E: number, F: number, firstDay: string = '0901', lastDay: string = '0831') => {
    let datestrFrom: string = ''
    let datestrTo: string = ''
    let datestrVer: string = ''
    let verNumber = 1
    let groupVerNumber: { [index: string]: number } = {
        A, B, C, D, E, F
    }
    let sieData = ''
    let accountingDate = new Date('2010-01-01')
    datestrFrom = formatDateYYYYMMDD(vers[0].date).substring(0, 4) + firstDay
    datestrTo = formatDateYYYYMMDD(vers[0].date, 1).substring(0, 4) + lastDay
    sieData = ''
    sieData += '#FLAGGA 0\r\n'
    sieData += '#FORMAT PC8\r\n'
    sieData += '#SIETYP 4\r\n'
    ...
    for (let n = 0; n <= vers.length; n++) {
        ...
        if (n === vers.length || (sieData && ver && datestrVer > datestrTo)) {
            ...
            if (n === vers.length)
                break
            datestrFrom = formatDateYYYYMMDD(vers[n].date).substring(0, 4) + firstDay
            datestrTo = formatDateYYYYMMDD(vers[n].date, 1).substring(0, 4) + lastDay
            sieData = ''
            sieData += '#FLAGGA 0\r\n'
            sieData += '#FORMAT PC8\r\n'
            sieData += '#SIETYP 4\r\n'
        ...
        if (ver) {
            ...
            sieData += `\r\n#VER ${ver.type} ${groupVerNumber[ver.type]} ${formatDateYYYYMMDD(ver.date)} "${ver.description}" ${formatDateYYYYMMDD(accountingDate)}\r\n`
            sieData += '{\r\n'
